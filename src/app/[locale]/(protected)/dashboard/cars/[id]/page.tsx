...
import { useCar } from "@/features/auth/hooks/use-cars"; // Agora o hook existe!

export default function CarDetailsPage() {
  const { id } = useParams();
  const router = useRouter();
  
  // Chamada do hook corrigida
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-blue-600 tracking-tight">
                {carro?.model} {/* Ajustado para o nome que vem do Java */}
              </h1>
        ...
        <div className="bg-white p-10 rounded-xl shadow-lg border-t-8 border-t-[#003cff]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-8">
              {/* Ajustado os 'value' para as propriedades em inglês que seu Service envia */}
              <DetailItem icon={<Tag />} label="Marca" value={carro?.brand} />
              <DetailItem icon={<Car />} label="Modelo" value={carro?.model} />
              <DetailItem icon={<Palette />} label="Cor" value={carro?.color} />
              <DetailItem icon={<Calendar />} label="Ano de Fabricação" value={carro?.year} />
              <DetailItem 
                  icon={<Calendar />} 
                  label="Data de Cadastro" 
                  value={carro?.dateCreate
                    ? new Date(carro.dateCreate).toLocaleString('pt-BR') 
                    : "---"
                  } 
                />
            </div>
